...
export default function Chat() {
  ...
  async function loadHistory() {
    setLoading(true);
    const msgHistory = await fetch(`http://localhost:4111/api/${params.threadId}/messages`)
      .then(res => res.json())
      .catch(() => [])
  ...
  useEffect(() => {
    if (!params.threadId || loading) return;
    loadHistory();
  }, [])

  // If no threadId in URL, redirect to one
  useEffect(() => {
    if (!params.threadId) {
      navigate(`/chat/${threadId}`, { replace: true });
    }
  }, [params.threadId, threadId, navigate]);
